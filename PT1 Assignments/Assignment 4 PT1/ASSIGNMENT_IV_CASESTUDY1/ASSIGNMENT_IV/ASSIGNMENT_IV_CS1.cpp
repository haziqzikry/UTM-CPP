...
// function which calculate the bill from the input of energy consumption
double* calcbill(int energy[])
{
	//repeat the process from first energy value until the last energy value
    for (int i = 0; i < 10; i++)
    {
     ...
     return bill;	//return bill value for each subscript
}

// function which define the caution level from the voltage value
...
int main()
{
	...
	for (int i = 0; i < 10; i++)
	{
		...
		for (int j = 0; j <= text[i].length(); j++)
		{
			...
			{
				//get the string value that does not include the user names
				...
				else if ((isspace(text[i][j]) && isdigit(text[i][j-1]) || isdigit(text[i][j])))
				{
					if(isspace(text[i][j]))
					{
						...
						//get the Voltage value from the string input
						V[i] = atof(temp.c_str());
						temp = "";
					}
					else
					{	
						//get the remaining value from the string input
						temp.push_back(text[i][j]);
					}
				}
			}
		}
		//from the remaining string input, change the data type from string to int and set to energy's variable
		energy[i] = atof(temp.c_str());
		temp = "";	
	}
		//calling for bill's value from calcbill() function
        bill = calcbill(energy);
        //calling for voltage's value from calcvolt() function
		outputV = calcvolt(V);
		
		//repeat the output process from first user until the last user
